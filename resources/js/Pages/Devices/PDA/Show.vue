<template>
    ...
    <div>
        ...
        <Layout>
            ...
            <Link class=" mb-6 bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded" :href="`/register/create/${pda.device_id}`">Add a Register</Link>
            <div class=" mt-6 p-6 w-auto bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Device: {{registerdevice.sitealias}} - {{registerdevice.departmentalias}} - {{pda.device["inventory_number"]}}</h5>
                ...
                <p v-if="pda.device['comment'] == null" class="mb-3 font-normal text-gray-700 dark:text-gray-400">Comment: No comment </p>
                <p v-if="pda.device['comment'] == null" class="mb-3 font-normal text-gray-700 dark:text-gray-400">Comment: No comment</p>
                <p v-else class="mb-3 font-normal text-gray-700 dark:text-gray-400">Comment: {{pda.device["comment"] }}</p>
                ...
                <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">Status: {{pda.device["status"]}}</p>
